--[[
	...
	Thanks from LUMORIA to Sirius and Rayfield, and all contributors for this beautiful UI library that powers LUMORIA Hub <3
]]

hubName = "LUMORIA Hub"

if debugX then
	warn('Initialising '..hubName)
end
...
local function loadSettings()
	...
	if not success then 
		if writefile then
			warn(hubName..' had an issue accessing configuration saving capability.')
		end
	end
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn(..hubName' had an issue decoding the configuration file, please try delete the file and reopen '..hubName) return end
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn(hubName.." | Unable to find '"..FlagName.. "' in the save file.")
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn(hubName..' | Not seeing your actions in notifications?')
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.LoadingTitle ~= hubName then
		LoadingFrame.Version.Text = hubName
	end
